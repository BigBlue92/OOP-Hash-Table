...
private:
	struct Node {
		...
		// Initialize with a bid and calls default
		Node(Bid aBid) : Node() {
			bid = aBid;
		}
...
/**
 * Destructor: logic to free storage when class is destroyed
 */
HashTable::~HashTable() {
...
void HashTable::Insert(Bid bid) {
	...
	// If no entries are found for this key
	if (oldNode == nullptr) {
...
void HashTable::PrintAll() {
    ...
    //Iterate the nodes vector and prints data as it goes
    while (i < nodes.size()) {
    cout << nodes[i].bid.bidId << ": " << nodes[i].bid.title << " | " << nodes[i].bid.amount << " | "
...
/**
 * Search for the specified bidId and return bid
 *
 * @param bidId The bid id to search for
 */
...
void loadBids(string csvPath, HashTable* hashTable) {
    ...
    try {
        ...
        for (unsigned int i = 0; i < file.rowCount(); i++) {
            ...
            // cout << "Item: " << bid.title << ", Fund: " << bid.fund << ", Amount: " << bid.amount << "ID: " << bid.bidId << endl;
